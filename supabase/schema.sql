...
create table if not exists participants (
  ...
  practice_finished_at timestamptz,
  valid_started_at timestamptz,
  valid_finished_at timestamptz,
...
create table if not exists attempts (
  ...
  mode text not null check (mode in ('practice','valid')),
...
-- Browser access happens only through SECURITY DEFINER RPC functions.
...
begin
  ...
  if p.id is null or crypt(p_pin, p.pin_hash) <> p.pin_hash then raise exception 'Nome ou PIN inválido'; end if;

  if p_mode='practice' and p.practice_started_at is not null then raise exception 'Teste já utilizado'; end if;
  if p_mode='valid' and p.valid_started_at is not null then raise exception 'Tentativa válida já utilizada'; end if;

  if p_mode='valid' then
    select coalesce(question_ids,'{}') into exclude_ids from attempts
      where participant_id=p.id and mode='practice';
  end if;
  ...
  insert into attempts(participant_id,mode,question_ids,current_question_started_at)
  values(p.id,p_mode,ids,now()) returning * into a;

  if p_mode='practice' then update participants set practice_started_at=now() where id=p.id;
  else update participants set valid_started_at=now() where id=p.id; end if;
...
language plpgsql security definer set search_path = public, extensions
as $$
declare a attempts; q questions; qid integer; shuffled jsonb; idx integer[];
begin
  select * into a from attempts where token=p_token;
  if a.id is null then raise exception 'Tentativa inválida'; end if;
  if a.finished_at is not null then return jsonb_build_object('finished',true); end if;
  if a.current_index >= 30 then return jsonb_build_object('finished',true); end if;

  qid := a.question_ids[a.current_index+1];
  select * into q from questions where id=qid;

  -- Start the 7s clock when the question is requested.
  update attempts set current_question_started_at=now() where id=a.id;

  -- Keep original option order in MVP. Correct index is never returned.
  return jsonb_build_object(
    'finished',false,'number',a.current_index+1,'total',30,
...
as $$
declare
  a attempts; q questions; qid integer; ms integer; ok boolean; weight numeric;
begin
  ...
  ok := (p_choice is not null and p_choice=q.answer);
  ...
  if a.current_index+1 >= 30 then
    update attempts set finished_at=now() where id=a.id;
    if a.mode='practice' then update participants set practice_finished_at=now() where id=a.participant_id;
    else update participants set valid_finished_at=now() where id=a.participant_id; end if;
...
returns table(place bigint, name text, score numeric, correct_count integer)
...
as $$
  ...
  order by a.score desc,a.correct_time_ms asc;
$$;
